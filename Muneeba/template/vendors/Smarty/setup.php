...
class Smarty_MyApp extends Smarty
{
    ...
    function render($view, $controller)
    {
        ...
        {
            $this->assign('file', '../view.tpl');
            $this->display('/var/www/html/training/training/Muneeba/template/app/views/layouts/login.tpl');
        } elseif ($view == 'dashboard') {
            //echo $view;
            //$this->display('../app/views/' . $controller . '/' . $view . '.tpl');
            $this->assign('file', '../' . $view . '.tpl');
            $this->display('/var/www/html/training/training/Muneeba/template/app/views/layouts/default.tpl');
        } else {
            $this->assign('file', '../'. $controller . '/' . $view . '.tpl');
            $this->display('/var/www/html/training/training/Muneeba/template/app/views/layouts/default.tpl');
        }
